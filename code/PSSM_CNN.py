...
def SMP2LIST(SMPFILE,open):
    ...
    for i in f.readlines():
        ...
        if counter >= 4 and counter <= 93:
            for j in range(20):
                temp.append(str(list(i)[9+3*j:9+3*j+1]).replace('[',"").replace(']',"").replace("'","").replace(' ','')+str(list(i)[9+3*j+1:9+3*j+2]).replace('[',"").replace(']',"").replace("'","").replace(' ','')+str(list(i)[9+3*j+2:9+3*j+3]).replace('[',"").replace(']',"").replace("'","").replace(' ',''))
                # print(type(temp[0]))
                # print(temp)
                # print(filename)
                # print(int(temp[0]))
...
for i in range(0,len(Rat_matrix)):
    for j in range(0,SEQ_SPLIT_LEN):
        for k in range(0,AMINO_LEN):
            try:
                PSSM_SEQ[i][j][k] = NUM2EXP(Rat_matrix[i][j][k])
                #print("success",Rat_matrix[i][j][k])
            except Exception as e:
                print("ERROR: "+Rat_matrix[i][j][k])
            ...
            #     #print("no error")
            #     #print(Rat_matrix[i][j][k],'->',PSSM_SEQ[i][j][k])
            # #異常情況 -6-10 or -3-4
            # else:
...
for i in range(1):
    ...
    model.add(MaxPooling1D(pool_size=24, strides=32, padding='valid'))
    model.add(Flatten())
    ...
    model.compile(loss=keras.losses.categorical_crossentropy,
                  optimizer='adam',
                  metrics=['accuracy'])
    train_history = model.fit(x=PSSM_SEQ,y=PSSM_LABEL,validation_split=0.2,epochs=100,batch_size=200,verbose=2)
